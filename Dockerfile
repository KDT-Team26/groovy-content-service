...
# libs/client-common을 함께 봐야 ':services:content-service:bootJar'가 성립한다).
...
RUN chmod +x gradlew && ./gradlew :services:content-service:bootJar --no-daemon -x test
...
COPY --from=build /workspace/services/content-service/build/libs/*-SNAPSHOT.jar app.jar
